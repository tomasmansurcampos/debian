...
ESSENTIAL_PACKAGES="build-essential fonts-noto-color-emoji firmware-linux-nonfree adb fastboot exfatprogs exfat-fuse rsync screen minicom dnsutils nmap stubby man nano gnupg gpgv gcc make cmake nasm gdb python-is-python3 stubby curl wget git binutils tcpdump lynx lsb-release htop bmon locales-all ascii ipcalc sipcalc rar unrar zip unzip p7zip p7zip-full p7zip-rar ffmpeg flac libavcodec-extra sox libsox-fmt-all"

PACKAGES="gnome-tweaks flatpak vlc spek audacity ardour keepassxc geany putty qbittorrent libreoffice libreoffice-gnome libreoffice-l10n-es wireshark"
